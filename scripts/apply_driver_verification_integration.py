...
# Settings module
p = 'deploy/cpanel/admin/settings.php'
s = read(p)
s = replace_once(
    s,
...
# Core approved-driver KYC gate
p = 'deploy/cpanel/rado-system/lib/app.php'
s = read(p)
...
# Prevent manual admin approval bypass and link to KYC center
p = 'deploy/cpanel/admin/drivers.php'
s = read(p)
s = replace_once(
    s,
...
# Driver session fields
p = 'apps/driver/lib/driver_platform.dart'
s = read(p)
...
# Driver app KYC entry points
p = 'apps/driver/lib/advanced_driver.dart'
s = read(p)
s = replace_once(
    s,
...
''',
  ...
  @override
''',
    'verification navigation method',
)
s = replace_once(
    s,
...
# Suspend approved driver automatically if a required document expires
p = 'deploy/cpanel/rado-system/lib/tick.php'
s = read(p)
s = replace_once(
    s,
    "    $pdo = rado_db();\n    $pdo->beginTransaction();",
...
# Dynamic progress honors optional checks
p = 'deploy/cpanel/rado-system/lib/api_ir.php'
s = read(p)
old = '''    $total = 8;
    $done = 0;
