...
function radoUpdaterHttpGet(string $url, array $config): string {
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_TIMEOUT => 60,
        CURLOPT_HTTPHEADER => ['Accept: application/vnd.github+json', 'User-Agent: ' . $config['user_agent']],
    ]);
    $body = curl_exec($ch);
...
function radoUpdaterAsset(array $assets, string $name): ?array {
    foreach ($assets as $asset) if (($asset['name'] ?? '') === $name) return $asset;
    return null;
}

function radoUpdaterPreserve(string $rel, array $config): bool {
...
function radoUpdaterRemoveTree(string $path): void {
...
function radoUpdaterInstallCore(string $root, string $stateDir, array $config, array $assets, array $meta, string $tag): void {
    ...
    foreach (['index.php','rado-system/lib/app.php','rado-system/lib/migrate.php','rado-system/lib/tick.php','rado-system/bin/rado-migrate.php','rado-system/bin/rado-update.php','rado-system/bin/rado-update-core.php'] as $required) {
...
try {
    ...
    try {
        ...
        @unlink($stateDir . '/tick-errors.log');
    } catch (Throwable $tickError) {
        @file_put_contents($stateDir . '/tick-errors.log', '[' . rado_jalali_datetime(null, true) . '] ' . $tickError->getMessage() . "\n", FILE_APPEND | LOCK_EX);
    }
    ...
    $line = '[' . rado_jalali_datetime(null, true) . '] ' . $e->getMessage() . "\n";
    @file_put_contents($stateDir . '/last_error.log', $line, FILE_APPEND | LOCK_EX);
    @file_put_contents($stateDir . '/last_error_current.log', $line, LOCK_EX);
    @file_put_contents($stateDir . '/update_status', "error\n", LOCK_EX);
