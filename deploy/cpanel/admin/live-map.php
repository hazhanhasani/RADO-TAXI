...
function passengerIcon(){return L.divIcon({className:'',iconSize:[36,36],iconAnchor:[18,18],html:'<div class="passenger-live">👤</div>'});}
...
function renderSnapshot(d){snapshotState={drivers:d.drivers||[],trips:d.trips||[]};const driverIds=new Set(snapshotState.drivers.map(x=>String(x.user_id)));for(const [id,m] of driverMarkers){if(!driverIds.has(id)){map.removeLayer(m);driverMarkers.delete(id);}}for(const x of snapshotState.drivers)upsertDriver(x,true);const tripIds=new Set(snapshotState.trips.map(x=>String(x.id)));const passengerIds=new Set();for(const [id,g] of tripLayers){if(!tripIds.has(id)){map.removeLayer(g);tripLayers.delete(id);}}for(const t of snapshotState.trips){upsertTrip(t);if(t.passenger_lat!=null&&t.passenger_lng!=null){passengerIds.add(String(t.id));upsertPassenger(t,t.passenger_lat,t.passenger_lng,true);}}for(const [id,m] of passengerMarkers){if(!passengerIds.has(id)){map.removeLayer(m);passengerMarkers.delete(id);}}updateCounters();document.getElementById('liveTime').textContent=d.time||'';}
