...
try {
    ...
    if($online&&$lat!==null&&$lng!==null){
      ...
      // One useful realtime row per GPS sample. Active drivers publish to their
      // trip channel so both passenger and operations can consume it. Free
      // drivers publish only to the operations channel. The driver's private
      // channel is reserved for offers/state events and is not polluted by GPS.
      ...
      if($activeTripId!==false){
        ...
        rado_platform_event($pdo,'ops:drivers','driver_location',$locationPayload,120);
      }
    }else{
      rado_platform_event($pdo,'ops:drivers','driver_location',['driver_id'=>$driverId,'online'=>false],600);
    }
