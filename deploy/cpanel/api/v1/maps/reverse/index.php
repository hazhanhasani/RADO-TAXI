...
$cacheDir = $root . '/rado-system/state/reverse-geocoding-v3';
...
if(!is_numeric($latRaw)||!is_numeric($lngRaw))respond(422,['ok'=>false,'error'=>'invalid_coordinates']);
$lat=(float)$latRaw;$lng=(float)$lngRaw;
if($lat < -90 || $lat > 90 || $lng < -180 || $lng > 180)respond(422,['ok'=>false,'error'=>'coordinates_out_of_range']);

$secrets=require $secretsFile;
$apiKey=trim((string)($secrets['neshan_service_api_key']??$secrets['neshan_reverse_api_key']??$secrets['neshan_map_key']??''));
if($apiKey==='')respond(503,['ok'=>false,'error'=>'neshan_service_api_key_missing']);

@mkdir($cacheDir,0755,true);
$cacheKey=hash('sha256','v3|'.number_format($lat,5,'.','').','.number_format($lng,5,'.',''));
$cacheFile=$cacheDir.'/'.$cacheKey.'.json';
if(is_file($cacheFile)&&filemtime($cacheFile)>time()-2592000){
    $cached=json_decode((string)file_get_contents($cacheFile),true);
    if(is_array($cached)&&!empty($cached['formatted_address'])){$cached['cached']=true;respond(200,$cached);}
}
...
if($body===false||$status<200||$status>=300)respond(502,['ok'=>false,'error'=>'neshan_reverse_failed','upstream_status'=>$status,'detail'=>$error!==''?$error:null]);
$data=json_decode((string)$body,true);if(!is_array($data))respond(502,['ok'=>false,'error'=>'invalid_neshan_response']);

$place=trim((string)($data['place']??$data['title']??$data['name']??''));
...
$display=$address;
if($place!=='')$display=$display===''?$place:(mb_stripos($display,$place,0,'UTF-8')===false?$place.'، '.$display:$display);
if($display==='')respond(404,['ok'=>false,'error'=>'address_not_found']);
